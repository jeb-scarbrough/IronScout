...
| Q6.4.1 | Price Evaluation Batching | Per-row DB reads are forbidden (self-inflicted outage at 50K rows). Batch-fetch last prices per chunk using `DISTINCT ON`. Maintain run-local `lastPriceCache` updated after each batch insert. Dedupe uses global prescan winner selection before chunk processing. |
