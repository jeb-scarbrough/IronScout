...
-- Enable pg_trgm extension for trigram text search (required for gin_trgm_ops)
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- Ensure legacy columns exist for index creation (shadow DBs may not have them)
ALTER TABLE "products" ADD COLUMN IF NOT EXISTS "grainWeight" INTEGER;
ALTER TABLE "products" ADD COLUMN IF NOT EXISTS "caseMaterial" TEXT;
ALTER TABLE "products" ADD COLUMN IF NOT EXISTS "roundCount" INTEGER;
...
-- Ensure AlertRuleType enum and alert columns exist before alert indexes
DO $$
BEGIN
...
ALTER TABLE "alerts" ADD COLUMN IF NOT EXISTS "ruleType" "AlertRuleType";
ALTER TABLE "alerts" ADD COLUMN IF NOT EXISTS "isEnabled" BOOLEAN NOT NULL DEFAULT true;
...
-- 2. Retailer lookup
CREATE INDEX IF NOT EXISTS idx_prices_retailer_id
...
CREATE INDEX IF NOT EXISTS idx_prices_product_retailer_date
...
CREATE INDEX IF NOT EXISTS idx_prices_price_stock
...
CREATE INDEX IF NOT EXISTS idx_prices_product_price_stock
...
CREATE INDEX IF NOT EXISTS idx_retailers_website
...
CREATE INDEX IF NOT EXISTS idx_retailers_tier_name
...
ON alerts("userId", "isEnabled") WHERE "isEnabled" = true;
...
CREATE INDEX IF NOT EXISTS idx_alerts_user_product_type
ON alerts("userId", "productId", "ruleType");
...
CREATE INDEX IF NOT EXISTS idx_executions_source_id
...
CREATE INDEX IF NOT EXISTS idx_executions_source_status
...
CREATE INDEX IF NOT EXISTS idx_executions_source_started
...
-- 5. Event filtering
CREATE INDEX IF NOT EXISTS idx_execution_logs_event
ON execution_logs(event);

-- 6. Composite for execution + level (finding errors for an execution)
CREATE INDEX IF NOT EXISTS idx_execution_logs_exec_level
...
CREATE INDEX IF NOT EXISTS idx_subscriptions_retailer_id
...
CREATE INDEX IF NOT EXISTS idx_advertisements_is_active
ON advertisements("isActive") WHERE "isActive" = true;

-- 2. Retailer ads lookup
CREATE INDEX IF NOT EXISTS idx_advertisements_retailer_id
ON advertisements("retailerId");

-- 3. Composite for active retailer ads
CREATE INDEX IF NOT EXISTS idx_advertisements_retailer_active
