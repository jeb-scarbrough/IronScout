...
function formatJson(entry: LogEntry): string {
  ...
  for (const [key, value] of Object.entries(truncated)) {
    if (typeof value === 'string' && value.length > 500) {
      (truncated as Record<string, unknown>)[key] = value.slice(0, 500) + `... [truncated from ${value.length} chars]`
    }
  }
...
export function sanitizeUrlForLogging(url: string | URL | null | undefined): string {
  ...
  try {
    ...
    sanitized = sanitized.replace(
      /([?&])(password|pass|pwd|secret|token|access_token|refresh_token|api_key|apikey|key|auth|authorization|bearer|credential|session|jwt)=([^&]*)/gi,
      '$1$2=[REDACTED]'
    )
