...
const shouldRunTier = (tier: MerchantTier): boolean => {
...
function describeTier(tier: MerchantTier): string {
...
describe.skipIf(!!RUN_TIER)('Cross-Tier Performance Comparison', () => {
  ...
  it('demonstrates linear scaling across tiers', async () => {
    const tiers: MerchantTier[] = ['hobbyist', 'serious', 'national']
...
describe.skipIf(!ENABLE_MEMORY_TESTS)('Memory Usage Tests', () => {
  ...
  it('tracks memory usage for large catalogs', async () => {
    ...
    Object.entries(memoryResults).forEach(([size, mem]) => {
      console.log(`size=${size} memoryMb=${mem}`)
    })
