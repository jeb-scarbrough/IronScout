...
describe('Dealer Feed Scheduling', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  describe('scheduleDealerFeeds', () => {
    it('should find all enabled feeds from active dealers', async () => {
      const feeds = [
  ...
  describe('Immediate Feed Ingestion', () => {
    it('should find feed with dealer info', async () => {
      ...
      const result = await mockFeedFindUnique({
        where: { id: 'feed-123' },
        include: { dealer: { select: { id: true, status: true, businessName: true } } },
      })

      expect(result).toBeDefined()
      expect(result.dealer.status).toBe('ACTIVE')
    })
    ...
    it('should throw if dealer is not active', async () => {
      const feed = createMockFeed({ dealer: { id: 'dealer-456', status: 'SUSPENDED' } })

      expect(feed.dealer.status).not.toBe('ACTIVE')
    })
