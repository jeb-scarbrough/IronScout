...
import { runDataIntegrityChecks, type DataIntegrityResults, type IntegrityCheckResult } from './actions';
...
export function DataIntegritySettings() {
  ...
  const handleRunChecks = async () => {
    ...
    const response = await runDataIntegrityChecks();
        ...
        <div>
          {results && (
            <p className="text-sm text-gray-600">
              Last checked: {results.lastChecked.toLocaleString()}
            </p>
          )}
        </div>
        <button
      ...
      {!results && !loading && (
        <div className="p-8 text-center bg-gray-50 rounded-lg border border-gray-200">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Checks include: pricing snapshot alignment, orphaned records, FK integrity
          </p>
        </div>
      )}
    </div>
