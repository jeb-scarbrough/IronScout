...
/**
 * Get feeds for a merchant with their status
 */
export async function getMerchantFeeds(merchantId: string) {
  const session = await getAdminSession();
  ...
  try {
    ...
    loggers.merchants.error('Failed to get merchant feeds', { merchantId }, error instanceof Error ? error : new Error(String(error)));
