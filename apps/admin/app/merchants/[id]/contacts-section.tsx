...
export function ContactsSection({ merchantId, contacts }: ContactsSectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let result;
      if (editingContact) {
        result = await updateMerchantContact(editingContact.id, merchantId, cleanedData);
      } else {
        result = await createMerchantContact(merchantId, cleanedData);
      }
  ...
  const handleTransferOwnership = async () => {
    ...
    try {
      const result = await transferAccountOwnership(merchantId, selectedContactForTransfer.id);
      if (result.success) {
  ...
  const handleSetOwner = async () => {
    ...
    try {
      const result = await setAccountOwner(merchantId, selectedContactForOwner.id);
      if (result.success) {
  ...
  const handleDelete = async (contact: Contact) => {
    ...
    setIsLoading(true);
    try {
      const result = await deleteMerchantContact(contact.id, merchantId);
      if (result.success) {
        router.refresh();
      } else {
              ...
              <div className="mb-6">
                ...
                <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-700">
                  <p>
                    ...
                    The new owner will have full control of this merchant account.
                  </p>
                </div>
              </div>
