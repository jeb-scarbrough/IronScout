...
export function FeedsSection({ merchantId, feeds, subscriptionStatus }: FeedsSectionProps) {
  ...
  async function handleTriggerFeed(feedId: string) {
    ...
    try {
      const result = await triggerManualFeedRun(merchantId, feedId);
      if (result.success) {
        setMessage({
          type: 'success',
  ...
  if (feeds.length === 0) {
    return (
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <p className="text-gray-500 text-sm">No feeds configured for this merchant.</p>
      </div>
    );
  }
      ...
      {isSubscriptionExpired && (
        <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
          <p className="text-sm text-amber-800">
            <strong>Note:</strong> This merchant&apos;s subscription has {subscriptionStatus.toLowerCase()}.
