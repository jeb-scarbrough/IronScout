...
export function DealerActions({ dealer }: DealerActionsProps) {
  ...
  const handleAction = async (action: 'approve' | 'suspend' | 'reactivate') => {
    const messages = {
      approve: `Approve ${dealer.businessName}? They will receive an email notification.`,
      suspend: `Suspend ${dealer.businessName}? They will lose access immediately.`,
      reactivate: `Reactivate ${dealer.businessName}? They will receive an email notification.`,
    };
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`/api/dealers/${dealer.id}/${action}`, {
        method: 'POST',
      });
