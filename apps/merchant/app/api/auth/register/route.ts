...
export async function POST(request: Request) {
  ...
  try {
    ...
    const emailResult = await sendMerchantVerificationEmail(
      result.merchantUser!.email,
      result.merchant!.businessName,
      result.merchantUser!.verifyToken!
    );
    ...
    // Notify admin about new merchant signup (email + Slack)
    reqLogger.debug('Sending admin notification');
    const notifyResult = await notifyNewMerchantSignup({
