...
export async function createCheckoutSession(
  ...
  try {
    ...
    const priceIdEnvVar =
      planId === 'pro'
        ? process.env.STRIPE_PRICE_ID_MERCHANT_PRO_MONTHLY
        : process.env.STRIPE_PRICE_ID_MERCHANT_STANDARD_MONTHLY;
    ...
    const baseUrl = process.env.NEXT_PUBLIC_MERCHANT_URL || 'https://merchant.ironscout.ai';
    const successUrl = `${baseUrl}/settings/billing?success=true`;
    const cancelUrl = `${baseUrl}/settings/billing?cancelled=true`;

    const response = await fetch(`${API_URL}/api/payments/merchant/create-checkout`, {
      method: 'POST',
      headers: {
 ...
 */
export async function createPortalSession(merchantId: string): Promise<PortalResult> {
  const session = await getSession();
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/api/payments/merchant/create-portal-session`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
