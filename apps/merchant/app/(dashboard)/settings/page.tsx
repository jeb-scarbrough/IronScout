...
export default async function SettingsPage() {
  ...
  const merchant = await prisma.merchants.findUnique({
    ...
    include: {
      merchant_notification_prefs: true,
      _count: {
        select: { merchant_contacts: { where: { isActive: true } } },
      },
    },
  });
  ...
  const getBillingStatus = () => {
    if (merchant.subscriptionStatus === 'ACTIVE') return 'Active';
    if (merchant.subscriptionStatus === 'EXPIRED') return 'Expired';
    if (merchant.subscriptionStatus === 'CANCELLED') return 'Cancelled';
    if (merchant.subscriptionStatus === 'SUSPENDED') return 'Suspended';
    return 'Not Set';
  };
  ...
  const settingsSections = [
    ...
    },
    {
      ...
      status: merchant._count?.merchant_contacts > 0 ? `${merchant._count.merchant_contacts} contact${merchant._count.merchant_contacts !== 1 ? 's' : ''}` : 'Not Set',
    },
    {
      title: 'Shipping',
      ...
      status: merchant.shippingType === 'UNKNOWN' ? 'Not Set' : 'Complete',
    },
    {
      title: 'Pixel Setup',
      ...
      icon: Code,
      href: '/settings/pixel',
      status: merchant.pixelEnabled ? 'Active' : 'Not Set',
    },
    {
      title: 'Notifications',
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
                  merchant.tier === 'FOUNDING' ? 'bg-purple-100 text-purple-700' :
                  merchant.tier === 'PRO' ? 'bg-blue-100 text-blue-700' :
