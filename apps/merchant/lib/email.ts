/**
 * Email Service for Merchant Portal
 *
...
const BASE_URL = process.env.NEXT_PUBLIC_MERCHANT_URL || process.env.NEXT_PUBLIC_DEALER_URL || 'https://merchant.ironscout.ai';
...
/**
 * Send verification email to newly registered merchant
 */
...
/**
 * Send admin notification when new merchant registers
 */
export async function sendAdminNewMerchantNotification(
  ...
  const emailLogger = logger.child({
    action: 'sendAdminNewMerchantNotification',
    merchantEmail,
    businessName
  });
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="text-align: center; margin-bottom: 30px;">
    <h1 style="color: #111; font-size: 24px; margin: 0;">IronScout</h1>
    <p style="color: #666; font-size: 14px; margin: 5px 0 0 0;">Merchant Management</p>
  </div>

  <div style="background: #fef3c7; border: 1px solid #f59e0b; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
    <p style="margin: 0; font-weight: 600; color: #92400e;">⏳ New Merchant Awaiting Approval</p>
  </div>
  ...
  <div style="background: #f9fafb; border-radius: 8px; padding: 25px; margin: 20px 0;">
    ...
    <table style="width: 100%; border-collapse: collapse;">
      ...
      <tr>
        ...
        <td style="padding: 8px 0; color: #111;"><a href="mailto:${merchantEmail}" style="color: #2563eb;">${merchantEmail}</a></td>
      </tr>
      ${phone ? `
      <tr>
  ...
  <div style="text-align: center; margin: 30px 0;">
    <a href="${merchantDetailUrl}" style="display: inline-block; background: #111; color: #fff; text-decoration: none; padding: 14px 30px; border-radius: 6px; font-weight: 600; font-size: 16px;">Review & Approve</a>
  </div>

  <div style="background: #f0f9ff; border: 1px solid #0ea5e9; border-radius: 8px; padding: 15px; margin: 20px 0;">
    <p style="margin: 0; font-size: 14px; color: #0369a1;">💡 <strong>Tip:</strong> Check the merchant's website to verify they are a legitimate firearms retailer before approving.</p>
  </div>

  <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 30px 0;">

  <div style="text-align: center; color: #888; font-size: 12px;">
    <p style="margin: 0;">Merchant ID: ${merchantId}</p>
    <p style="margin: 10px 0 0 0;">&copy; ${new Date().getFullYear()} IronScout. All rights reserved.</p>
      ...
      `,
      text: `
New Merchant Registration - Awaiting Approval

Business: ${businessName}
Contact: ${contactName}
Email: ${merchantEmail}${phone ? `\nPhone: ${phone}` : ''}
Website: ${websiteUrl}
Status: PENDING

Review and approve this merchant:
${merchantDetailUrl}

Tip: Check the merchant's website to verify they are a legitimate firearms retailer before approving.

---
Merchant ID: ${merchantId}
IronScout Merchant Management
      `.trim(),
    });
