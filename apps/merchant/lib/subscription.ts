/**
 * Merchant Portal Subscription Check
 *
...
import type { merchants } from '@ironscout/db';

// Type alias for the merchant entity
type Merchant = merchants;
...
function calculateSubscriptionStatus(merchant: Merchant): SubscriptionStatus {
  ...
  const subscriptionStatus = merchant.subscriptionStatus;
  ...
  if (subscriptionStatus === 'ACTIVE') {
    ...
    const now = new Date();
    const graceDays = merchant.subscriptionGraceDays ?? DEFAULT_GRACE_DAYS;
...
export type MerchantTier = 'STANDARD' | 'PRO' | 'FOUNDING';
...
/**
 * Check if a merchant tier has access to PRO features
...
/**
 * Check if a merchant has access to a specific PRO feature
 */
...
/** @deprecated Use MerchantTier instead */
export type DealerTier = MerchantTier;
