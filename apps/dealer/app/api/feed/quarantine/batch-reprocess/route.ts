...
export async function POST(request: Request) {
  ...
  try {
    ...
    const records = await prisma.quarantinedRecord.findMany({
      where: {
        id: { in: recordIds },
    ...
    for (const record of records) {
      try {
        ...
        // Generate SKU hash and create DealerSku
        ...
        await prisma.quarantinedRecord.update({
          where: { id: record.id },
