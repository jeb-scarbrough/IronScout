 ...
 * Submit a product suggestion for a dealer SKU that has no matching canonical product
 */
export async function POST(
  ...
  try {
    // Verify ownership
    const sku = await prisma.dealerSku.findFirst({
...
export async function GET(
  ...
  try {
    ...
    const suggestion = await prisma.productSuggestion.findFirst({
      where: { dealerSkuId: id },
