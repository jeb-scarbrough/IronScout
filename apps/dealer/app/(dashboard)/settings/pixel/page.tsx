...
export default async function PixelSettingsPage() {
  ...
  const pixelCode = dealer.pixelApiKey ? `
...
`.trim() : '';
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Status */}
      <div className={`rounded-lg p-4 ${dealer.pixelEnabled ? 'bg-green-50 border border-green-200' : 'bg-yellow-50 border border-yellow-200'}`}>
      ...
      <div className="rounded-lg bg-white shadow">
        <div className="px-4 py-5 sm:p-6">
          ...
          <PixelActions 
            dealerId={dealer.id} 
            apiKey={dealer.pixelApiKey} 
            pixelEnabled={dealer.pixelEnabled}
          />
        </div>
      </div>
